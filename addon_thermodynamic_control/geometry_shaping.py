...
class LandscapeShaper:
    ...
    def unsafe_curvature(self, t: torch.Tensor, safety_inputs: torch.Tensor) -> torch.Tensor:
        """
        Rayleigh quotient along safety gradient direction: v^T H_safety v (v = ∇L_safety / ‖∇L_safety‖).

        t must already require grad so the result stays in the computation graph
        and contributes to the shaping gradient via double backprop.
        """
        ...
        norm = LA.norm(grad.detach())
        if norm < 1e-8:
            return torch.tensor(0.0)
        direction = (grad.detach() / norm)
    ...
    def shaping_step(self, theta, safety_inputs, task_inputs, task_labels):
        ...
        curv = self.unsafe_curvature(t, safety_inputs)   # pass t, not theta
...
class SelfRegularizingObjective:
    ...
    def expected_repair_energy(self, t: torch.Tensor, safety_inputs: torch.Tensor) -> torch.Tensor:
        """
        sigma_drift^2 * Tr(H_unsafe) via Hutchinson estimator.

        t must already require grad. hvp.detach() is used for the trace probe
        so the Hutchinson sum itself stays in the graph only through its
        dependence on t via grad (double backprop through the Hessian).
        """
        ref_out = self.model_fn(safety_inputs, self.theta_ref).detach()
        kl = F.kl_div(
            F.log_softmax(self.model_fn(safety_inputs, t), dim=-1),
        ...
        for _ in range(self.n_probes):
            ...
            trace = trace + (v.detach() * hvp).sum() / self.n_probes
    ...
    def compute_gradient(self, theta, safety_inputs, task_inputs, task_labels):
        ...
        t = theta.detach().requires_grad_(True)
        task_loss = F.cross_entropy(self.model_fn(task_inputs, t), task_labels)
        exp_repair = self.expected_repair_energy(t, safety_inputs)  # pass t, not theta
