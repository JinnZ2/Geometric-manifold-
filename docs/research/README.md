...
| `OPEN_QUESTIONS.md` | This repo | Six questions to be settled by measurement rather than preference, each with a named experiment and an honest status. Q2 answered, Q5 and Q7 tested (Q7 half), Q1/Q4/Q6 open. |
...
| `sims/dark_constraint/` | Dark matter as a shadow shape: can an *unmodelled* constraint be detected, and when? | **REFUTED as posed**, but H1 is clean and important: at one load case the degeneracy is **exact** — the visible model fits to machine precision and reports 0.014–0.067 of *fictitious* residuals on healthy edges. The detection half is untested; my null was structurally zero. |
| `sims/shape_shadow/` | Do faults hide in the drill-down projection? (12 edge residuals → 6 vertex magnitudes) | **REFUTED** as posed — concealment is real but shallow (0.21×→0.66×, decaying with fault size), so the 12→6 projection does **not** leave half of fault space invisible. Supports notes 14's drill-down more than it qualifies it. Two design errors of mine recorded in `FINDING.md`. |
